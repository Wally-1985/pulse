...
exports.updateSettings = async (req, res) => {
  ...
  try {
    ...
    if (Object.keys(updates).some(k => k.startsWith('smtp_'))) resetTransporter();
...
exports.getNonWorkingDates = async (req, res) => {
  try {
    ...
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: 'Failed to fetch non-working dates' }); }
};
...
exports.createNonWorkingDate = async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ id });
  } catch (err) { res.status(500).json({ error: 'Failed to create non-working date' }); }
};
...
exports.getAuditLogs = async (req, res) => {
  ...
  try {
    ...
    const result = await query(
      'SELECT al.*, u.email, u.first_name, u.last_name FROM audit_logs al LEFT JOIN users u ON u.id = al.user_id WHERE ' + where + ' ORDER BY al.created_at DESC LIMIT $' + (params.length + 1) + ' OFFSET $' + (params.length + 2),
      [...params, limit, offset]
    );
    res.json({ total: parseInt(countResult.rows[0].count), page: parseInt(page), limit: parseInt(limit), logs: result.rows });
...
exports.exportAuditLogs = async (req, res) => {
  ...
  try {
    const result = await query(
      'SELECT al.created_at, u.email, al.role, al.action_type, al.entity_type, al.entity_id, al.success, al.ip_address FROM audit_logs al LEFT JOIN users u ON u.id = al.user_id WHERE ($1::date IS NULL OR al.created_at >= $1) AND ($2::date IS NULL OR al.created_at <= $2) ORDER BY al.created_at DESC',
      [from || null, to || null]
    );
    const headers = ['Timestamp','User','Role','Action','Entity Type','Entity ID','Success','IP'];
    const rows = result.rows.map(r => [r.created_at, r.email||'', r.role||'', r.action_type, r.entity_type||'', r.entity_id||'', r.success, r.ip_address||'']);
...
exports.runBackup = async (req, res) => {
  try {
    if (!fs_mod.existsSync(BACKUP_DIR)) fs_mod.mkdirSync(BACKUP_DIR, { recursive: true });
    const ts = new Date().toISOString().replace(/[:.]/g, '-');
    const backupName = 'pulse-backup-' + ts;
    const backupDir = path.join(BACKUP_DIR, backupName);
    fs_mod.mkdirSync(backupDir, { recursive: true });

    const { DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD } = process.env;
    const env = { ...process.env, PGPASSWORD: DB_PASSWORD };

    // 1. Database dump
    execSync('pg_dump -h ' + (DB_HOST||'localhost') + ' -p ' + (DB_PORT||'5432') + ' -U ' + DB_USER + ' -d ' + DB_NAME + ' -f "' + path.join(backupDir, 'database.sql') + '"', { env });

    // 2. App source files
    const appRoot = path.join(__dirname, '../../..');
    const srcItems = ['backend/src', 'frontend/src', 'backend/package.json', 'frontend/package.json'];
    for (const src of srcItems) {
      const srcPath = path.join(appRoot, src);
      if (fs_mod.existsSync(srcPath)) {
        const destPath = path.join(backupDir, 'app', src);
        fs_mod.mkdirSync(path.dirname(destPath), { recursive: true });
        execSync('cp -r "' + srcPath + '" "' + path.dirname(destPath) + '/"');
      }
    }

    // 3. .env config (without secrets - just keys)
    const envPath = path.join(appRoot, 'backend/.env');
    if (fs_mod.existsSync(envPath)) {
      const envContent = fs_mod.readFileSync(envPath, 'utf8')
        .split('\n').map(l => l.includes('PASSWORD') || l.includes('SECRET') || l.includes('PASS=') ? l.replace(/=.*/, '=***REDACTED***') : l).join('\n');
      fs_mod.writeFileSync(path.join(backupDir, 'env.redacted.txt'), envContent);
    }
    ...
    res.json({ message: 'Backup created', filename: backupName + '.zip', size: stat.size });
  } catch (err) {
...
exports.listBackups = async (req, res) => {
  try {
    if (!fs_mod.existsSync(BACKUP_DIR)) return res.json([]);
    const files = fs_mod.readdirSync(BACKUP_DIR)
      .filter(f => f.endsWith('.zip') || f.endsWith('.sql'))
      .map(f => { const stat = fs_mod.statSync(path.join(BACKUP_DIR, f)); return { filename: f, size: stat.size, createdAt: stat.birthtime }; })
...
exports.getSystemHealth = async (req, res) => {
  const health = { db: false, backupCount: 0, lastBackup: null };
  try { await query('SELECT 1'); health.db = true; } catch {}
  try {
    if (fs_mod.existsSync(BACKUP_DIR)) {
      const files = fs_mod.readdirSync(BACKUP_DIR).filter(f => f.endsWith('.zip') || f.endsWith('.sql'));
      health.backupCount = files.length;
      if (files.length) {
        const latest = files.sort().reverse()[0];
        health.lastBackup = fs_mod.statSync(path.join(BACKUP_DIR, latest)).birthtime;
      }
    }
  } catch {}
...
exports.getApiKeys = async (req, res) => {
  try {
    const result = await query('SELECT id, name, key_prefix, permissions, ip_restrictions, last_used_at, expires_at, created_at, revoked_at FROM api_keys ORDER BY created_at DESC');
    res.json(result.rows);
...
exports.createApiKey = async (req, res) => {
  ...
  try {
    ...
    const id = uuidv4();
    await query(
      'INSERT INTO api_keys (id, name, key_hash, key_prefix, permissions, ip_restrictions, created_by, expires_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)',
      [id, name, keyHash, keyPrefix, JSON.stringify(permissions || { read: true, write: false }), ipRestrictions || null, req.user.id, expiresAt || null]
    );
