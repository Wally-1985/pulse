              ...
              <div className="flex items-center gap-1 shrink-0">
                {!readOnly && onAddWorkItem && (
                  <button
                    ...
                    disabled={isAdded}
                    className={'text-[10px] px-1.5 py-0.5 rounded transition-colors ' + (isAdded
                      ? 'bg-green-500/20 text-green-400 cursor-default'
                      : 'bg-[var(--pulse-accent-soft)] text-[var(--pulse-accent)] hover:bg-[var(--pulse-accent)] hover:text-white')}
                    title={isAdded ? 'Added to entry' : 'Add to entry'}
                  >
                    {isAdded ? '✓ Added' : '+ Add'}
                  </button>
                )}
